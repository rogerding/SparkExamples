...
public class SqlQ04 {
    ...
    static String myQuery = "select loc, sum(sal) from emp JOIN dept on emp.deptno=dept.deptno group by loc";

    public static void main(String[] args) {

        if (args.length != 1) {
            System.out.println("Usage: SqlQ04 outputPath");
            System.exit(1);
        }
        String outPath = args[0];

        SparkConf sparkConf = new SparkConf().setAppName("Java-SqlQuery-04");
...
// spark-submit --master local[4] --class SparkSQLJava.SqlQ04 /home/cloudera/IdeaProjects/SparkExamples/target/SparkExamples-1.0-SNAPSHOT.jar hdfs://quickstart.cloudera:8020/user/cloudera/class/q004
