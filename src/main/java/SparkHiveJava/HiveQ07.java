 ...
 */
public class HiveQ07 {

    static int num_partition = 1;
    static String myQuery1 = "use emp";

    // Unsupported language features in query:
    // scala.NotImplementedError: No parse rules for ASTNode type: 305, text: 'J%' :
    static String myQuery2 = "select emp.ename, dept.dname from emp JOIN dept on emp.deptno=dept.deptno and emp.ename like 'J%'";

    public static void main(String[] args)  {

        if (args.length != 1) {
            System.out.println("Usage: HiveQ07 outputPath");
            System.exit(1);
        }
        String outPath = args[0];

        SparkConf sparkConf = new SparkConf().setAppName("Java-HiveQuery-07");
...
// spark-submit --master local[4] --class SparkHiveJava.HiveQ07 /home/cloudera/IdeaProjects/SparkExamples/target/SparkExamples-1.0-SNAPSHOT.jar hdfs://quickstart.cloudera:8020/user/cloudera/class/h07
