 ...
 */
public class HiveQ08 {

    static int num_partition = 1;
    static String myQuery1 = "use emp";

    // Unsupported language features in query: select ename, sal from emp order by sal desc limit 3
    // scala.NotImplementedError: No parse rules for ASTNode type: 294, text: 3 :
    // static String myQuery2 = "select ename, sal from emp order by sal desc limit 3";
    static String myQuery2 = "select ename, sal from emp order by sal desc";

    public static void main(String[] args)  {

        if (args.length != 1) {
            System.out.println("Usage: HiveQ08 outputPath");
            System.exit(1);
        }
        String outPath = args[0];

        SparkConf sparkConf = new SparkConf().setAppName("Java-HiveQuery-08");
...
// spark-submit --master local[4] --class SparkHiveJava.HiveQ08 /home/cloudera/IdeaProjects/SparkExamples/target/SparkExamples-1.0-SNAPSHOT.jar hdfs://quickstart.cloudera:8020/user/cloudera/class/h08
